...
//    Log[1].stamp_minutes = 9;
    ...
    struct PriceChange price_change_event_struct;
            ...
            case (ADJUST_PRICE_COMMAND):
                if (UARTMessagesWaiting >= 5)
                {
                    ...
                    if ( xSemaphoreTake(xStation_mutex, portMAX_DELAY) == pdTRUE )
                    {
                        ...
                        price_change_event_struct.type = byte_from_UART_queue;
                        ...
                        //price_change_event_struct.new_price = temp_new_price;

                        //xEventGroupSetBits(station_eventgroup, price_change_event_struct);

...
void LEDDriverTask (void * pvParameters)
{
    ...
    for (;;)
    {
        uxLEDBits = xEventGroupWaitBits(
                LEDs_eventgroup,
                pump_ON_event  | pump_OFF_event | shunt_ON_event | shunt_OFF_event,
                pdTRUE,
                pdFALSE,
                portMAX_DELAY );

        //GPIO_PORTF_DATA_R = 0x02;
        if (uxLEDBits & pump_ON_event)
        {
            GPIO_PORTF_DATA_R &= 0x0C; // Turn on RED LED
        }
        if (uxLEDBits & pump_OFF_event)
        {
            GPIO_PORTF_DATA_R |= 0x02; // Turn off RED LED
        }

        if (uxLEDBits & shunt_ON_event)
        {
            GPIO_PORTF_DATA_R &= 0x0A; // Turn on YELLOW LED
        }
        if (uxLEDBits & shunt_OFF_event)
        {
